...
-- Records attendance for a given event day, admin-only,
-- blocks duplicate check-ins per day at the DB level
-- =========================================================
...
BEGIN
    ...
    -- Only admins may check attendees in
    ...
    -- Find the attendee by their QR token
    ...
    -- Has this attendee already been checked in for this date?
    ...
    IF v_existing_id IS NOT NULL THEN
        -- Already checked in: return the existing record, flagged
        RETURN QUERY
        SELECT
            v_existing_id,
            v_attendee_id,
    ...
    RETURNING att.id, att.checked_in_at
    INTO v_new_id, v_new_checked_in_at;
...
-- Lock down execution: admins (authenticated) only.
-- Anonymous users must never be able to check people in.
...
-- Powers the daily headcount dashboard
-- =========================================================
...
BEGIN
    IF NOT public.is_admin() THEN
        RAISE EXCEPTION 'Only admins can view attendance summaries';
    END IF;
    ...
    WHERE attendance.event_date = p_event_date;
